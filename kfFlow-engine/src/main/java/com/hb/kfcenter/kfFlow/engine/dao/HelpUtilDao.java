...
import com.hb.kfcenter.kfFlow.engine.util.LogicMath;
 ...
 */
@Component
public class HelpUtilDao implements HelpUtilService {
	...
	public Map<String, String> getJsonStringToLevelMap(String jsonString) {
		...
		try {
			if (StringUtils.isNotEmpty(jsonString)) {
				...
				if (node != null) {
					result = new LinkedHashMap<String, String>(100);
					Iterator<String> iterator = node.fieldNames();
					while (iterator.hasNext()) {
						String _key = iterator.next();
						JsonNode _node = node.get(_key);
						if ("baseForm".equalsIgnoreCase(_key)) {
							_key += "s";
						}
						if (_node.isArray()) {
							Iterator<JsonNode> _iterator = _node.elements();
							while (_iterator.hasNext()) {
								JsonNode _nodelevel = _iterator.next();
								String _subKey = null;
								if (_nodelevel.has("colName")) {
									_subKey = _nodelevel.get("colName").asText();
								}else if (_nodelevel.has("id")) {
									_subKey = _nodelevel.get("id").asText();
								}
								
								String _value = _nodelevel.get("val").asText();
								result.put(_key+"."+_subKey, _value);
							}
						}
					}
				}
...
/*	public static void main(String[] args) {
	...
	private boolean executeExpression(RuleBean conditionBean,RuleBean paramBean,Map<String,String> paramJson) {
		boolean result = false;
		try {
			if (conditionBean != null && StringUtils.isNotEmpty( conditionBean.getType() ) && "condition".equalsIgnoreCase(conditionBean.getType()) &&
					StringUtils.isNotEmpty(conditionBean.getParaOne()) && StringUtils.isNotEmpty(conditionBean.getParaTwo()) &&
					paramBean != null && StringUtils.isNotEmpty( paramBean.getType() ) && "param".equalsIgnoreCase(paramBean.getType()) &&
					StringUtils.isNotEmpty(paramBean.getParaOne()) && StringUtils.isNotEmpty(paramBean.getParaTwo())) {
				
				String _paramStr = paramJson.containsKey(paramBean.getParaTwo())?paramJson.get( paramBean.getParaTwo() ):null;
				if (StringUtils.isNotEmpty(_paramStr)) {
					ExpressionService _service = LogicMath.init(_paramStr, conditionBean);
					if (_service != null)
						result = _service.runExpression();
				}else {
					return false;
				}
//				System.out.println(conditionBean.getType() + "####" + conditionBean.getParaOne() + "####" + conditionBean.getParaTwo());
//				System.out.println(paramBean.getType() + "####" + paramBean.getParaOne() + "####" + paramBean.getParaTwo());
			}
		} catch (Exception e) {
		}
		return result;
	...
	public boolean parseToExpression(RuleBean bean, List<RuleBean> list,Map<String,String> paramJson) {
		boolean result = false;
		List<RuleBean> actRules = getActList(bean,list,null);
		if (actRules != null && actRules.size() % 2 == 0) {
			for (int i =0 ;i<actRules.size();i++) {
				if ( i % 2 == 0) {
					result = executeExpression(actRules.get(i),actRules.get(i+1),paramJson);
					if (!result)
						break;
				}else {
					continue;
...
//			for(RuleBean _bean:actRules) {
//				System.out.println(_bean.getType() + "####" + _bean.getParaOne() + "####" + _bean.getParaTwo());
//			}
		}
		return result;
	}
